...
def handle(args):
    ...
    print("-" * 70)
    print(f"Transactions Categorized:     {cov['pct_txns']:>6.1f}% ({cov['count_labeled']}/{cov['count_total']})")
    print(f"Outbound Spending Categorized: {cov['pct_debit']:>6.1f}% (${cov['debit_labeled']:>12,.0f}/${cov['debit_total']:>12,.0f})")
    print(f"Inbound Income Categorized:    {cov['pct_credit']:>6.1f}% (${cov['credit_labeled']:>12,.0f}/${cov['credit_total']:>12,.0f})")
...
def register(subparsers):
    ...
    parser = subparsers.add_parser("coverage", help="Show coverage gaps & uncategorized transactions")
