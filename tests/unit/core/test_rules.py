...
def test_multiple_subsumed():
    keywords = ["A", "AB", "ABC", "ABCD"]
    result = dedupe_keywords(keywords)
    assert result == ["A"]


def test_no_subsumption():
    keywords = ["COLES", "WOOLWORTHS", "ALDI"]
    result = dedupe_keywords(keywords)
    assert result == ["ALDI", "COLES", "WOOLWORTHS"]
...
def test_load_empty_files(tmp_path):
    rules_dir = tmp_path / "rules"
    rules_dir.mkdir()
...
def test_load_comments_and_blanks(tmp_path):
    rules_dir = tmp_path / "rules"
    rules_dir.mkdir()
